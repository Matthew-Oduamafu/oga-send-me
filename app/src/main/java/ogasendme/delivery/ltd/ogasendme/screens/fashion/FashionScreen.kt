package ogasendme.delivery.ltd.ogasendme.screens.fashion
...
import ogasendme.delivery.ltd.ogasendme.data.fake.fake_data_fashion
...
private const val TAG = "FashionScreen"


@SuppressLint("UnusedMaterialScaffoldPaddingParameter")
@Composable
fun FashionScreen(navController: NavController) {
    Log.d(TAG, "Fashion: called")
    ...
    Scaffold(
        topBar = {
            OgaTopAppBar(
                title = stringResource(id = R.string.fashion_screen_lbl),
                bgColor = Color.White,
        ...
        Surface(modifier = Modifier.fillMaxSize()) {
            Column(
                ...
                SearchBox(label = stringResource(id = R.string.place_or_product_lbl), onSearch = {
                    Log.d(TAG, "FashionScreen: search query is $it")
                    searchItem(it)
                })
                ...
                LazyColumn(
                    ...
                    items(items = fake_data_fashion) { item ->
                        ItemOrProductCard(item = item, isFood = false) {
                            Log.d(TAG, "FashionScreen: item ${item.itemImage} clicked")
